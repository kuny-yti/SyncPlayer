...
QMPlay2CoreClass::QMPlay2CoreClass()
{
	qmplay2Core = this;
#ifdef Q_OS_WIN
    logFilePath = QDir::tempPath() + "/SyncPlayer.log";
#else
    logFilePath = QDir::tempPath() + "/SyncPlayer." + QString(getenv("USER")) + ".log";
#endif
#if defined Q_OS_MAC
...
void QMPlay2CoreClass::init(bool loadModules, const QString &_qmplay2Dir, const QString &_settingsDir)
{
	...
	{
#ifdef Q_OS_WIN
        settingsDir = QFileInfo(QSettings(QSettings::IniFormat, QSettings::UserScope, QString()).fileName()).absolutePath() + "/SyncPlayer/";
#else
		...
		foreach (const QFileInfo &fInfo, pluginsList)
			if (QLibrary::isLibrary(fInfo.filePath()))
			{
				...
				else
				{
					...
					{
#ifndef Q_OS_ANDROID
                        log(fInfo.fileName() + " - " + tr("invalid SyncPlayer library"), AddTimeToLog | ErrorLog | SaveLog);
#endif
					}
					else
					{
    ...
    settings = new Settings("SyncPlayer");
