...
static QStringList EnumExtensionKeys()
{
	...
	if (!RegOpenKeyW(HKEY_CURRENT_USER, L"Software\\Classes", &key))
	{
		...
		for (DWORD d = 0; !RegEnumKeyW(key, d, name, sizeof name / sizeof *name); ++d)
		{
            ...
            if (!SHGetValue(HKEY_CURRENT_USER, (WCHAR *)QString("Software\\Classes\\" + keyName).utf16(), NULL, NULL, value, &valueLen) && !wcsncmp(value, L"SyncPlayer", 7))
				keys += keyName;
		}
		RegCloseKey(key);
	}
...
void ModuleSettingsWidget::saveSettings()
{
	...
	if (associateB->isChecked())
	{
        ...
        CreateQMPlay2Key("SyncPlayer", QMPlay2Path, 1, tr("Enqueue in QMPlay2"));
		...
		for (int i = 0; i < extensionLW->count(); ++i)
		{
			...
			if (lWI->checkState() == Qt::Checked)
                AddStringToReg("." + lWI->text(), lWI->data(Qt::UserRole).toBool() ? "QMPlay2Playlist" : "SyncPlayer");
		}
	}
