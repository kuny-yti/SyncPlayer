...
void subtitle_updater::set(
        const subtitle_box& subtitle, qint64 timestamp,
...
void subtitle_updater::run()
{
    ...
    if (_subtitle.is_valid()
            && (_subtitle != _last_subtitle
                || (!_subtitle.is_constant() && _timestamp != _last_time)
    ...
    if (_buffer_changed) {
        size_t bufsize = _bb_w * _bb_h * sizeof(quint32);
        if (_buffer.size() < bufsize)
            _buffer.resize(bufsize);
        _renderer->render(_buffer.ptr<quint32>());
    }
}
