...
void input_handel::init(const video_frame &last_frame)
{
    ...
    if (last_frame.layout == LAYOUT_BGRA32)
    {
        for (int i = 0; i < (last_frame.stereo_layout == STEREO_MONO ? 1 : 2); i++)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, last_frame.size.width(),
                         last_frame.size.height(),
    ...
    else
    {
        ...
        if (last_frame.layout == LAYOUT_YUV422P)
        {
            _yuv_chroma.setWidth(2);
            need_chroma_filtering = true;
        }
        else if (last_frame.layout == LAYOUT_YUV420P)
        {
            _yuv_chroma = QSize(2,2);
            need_chroma_filtering = true;
        }
        bool type_u8 = (last_frame.value_range == RANGE_U8_FULL ||
                        last_frame.value_range == RANGE_U8_MPEG);
        ...
        for (int i = 0; i < (last_frame.stereo_layout == STEREO_MONO ? 1 : 2); i++)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, internal_format,
                    last_frame.raw_size.width(),
                    last_frame.raw_size.height(),
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            glTexImage2D(GL_TEXTURE_2D, 0, internal_format,
                    last_frame.raw_size.width() / _yuv_chroma.width(),
                    last_frame.raw_size.height() / _yuv_chroma.height(),
                    0, GL_LUMINANCE, type, NULL);
...
{
    return (_last_frame.size == current_frame.size
...
void input_handel::handel(const video_frame &frame)
{
    ...
    {
        bool type_u8 = (frame.value_range == RANGE_U8_FULL ||
                        frame.value_range == RANGE_U8_MPEG);
    ...
    for (int i = 0; i < 1; i++)
    {
        for (int plane = 0; plane < (frame.layout == LAYOUT_BGRA32 ? 1 : 3); plane++)
        {
            ...
            int w = frame.raw_size.width();
            int h = frame.raw_size.height();
            GLuint tex;
            int row_size;
            if (frame.layout == LAYOUT_BGRA32)
            {
