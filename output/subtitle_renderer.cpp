...
void subtitle_renderer::set_ass_parameters(const parameters &params)
{
    ...
    {
        overrides.push_back(QString("Default.Fontname=") + params.subtitle_font());
    }
    if (!params.subtitle_size_is_default())
    {
        overrides.push_back(QString("Default.Fontsize=") + QString::number(params.subtitle_size()));
    }
    if (!params.subtitle_color_is_default())
    {
        ...
        char buff[16] = {0};
        sprintf(buff,"&H%02x%02x%02x%02x", a, b, g, r);
        overrides.push_back(QString("Default.PrimaryColour=") + QString(buff));
        overrides.push_back(QString("Default.SecondaryColour=") + QString(buff));
    }
    if (!params.subtitle_shadow_is_default())
    {
        overrides.push_back(QString("Default.Shadow=")
                + (params.subtitle_shadow() == 0 ? "0" : "3"));
    }
    const char *ass_overrides[overrides.size() + 1];
    for (size_t i = 0; i < overrides.size(); i++)
    {
        ass_overrides[i] = overrides[i].toLatin1().data();
    }
        ...
        const parameters &params, int width, int height, float pixel_aspect_ratio)
{
    ...
    if (params.subtitle_encoding() != "")
    {
        try
        {
            conv_str = box.str.toUtf8();
        }
        catch (std::exception &e)
        {
            //msg::err(_("Subtitle character set conversion failed: %s"), e.what());
            conv_str = QString("Dialogue: 0,0:00:00.00,9:59:59.99,") + e.what();
        }
    }
    if (box.format == FORMAT_ASS)
    {
        ass_process_codec_private(_ass_track, const_cast<char *>(box.style.toLatin1().data()), box.style.length());
        ass_process_data(_ass_track, const_cast<char *>(conv_str.toLatin1().data()), conv_str.length());
    }
    else
    {
        ...
        QString style =
            ...
            "\n";
        ass_process_codec_private(_ass_track, const_cast<char *>(style.toLatin1().data()), style.length());
        // Convert text to ASS
        conv_str.replace("\r\n", "\\N");
        conv_str.replace("\n", "\\N");
        QString str = "Dialogue: 0,0:00:00.00,9:59:59.99," + conv_str;
        ass_process_data(_ass_track, const_cast<char *>(str.toLatin1().data()), str.length());
    }
